	...
	execute(message, args, con) {
        ...
        con.query(`SELECT * FROM Noodle WHERE id = ${target} ORDER BY id`, (err, rows) => {
            ...
            if (parseInt(curReceiver) + parseInt(amt) > bigIntMax) {
                message.channel.send(`<@${message.author.id}> gave <@${target}> ${amt} coins!`);
            ...
            message.channel.send(`<@${message.author.id}> gave <@${target}> ${commas(amt)} coins using admin permissions!`);
