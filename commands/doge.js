...
module.exports = {
    name: 'doge',
    description: 'Grabs a random post from r/doge',
    aliases: [],
    guildOnly: false,
    cooldown: 1,
    execute(message, args) {
        ...
        async function getRedditPost(){
            try{
                const response = await superagent.get('https://www.reddit.com/r/doge/random.json');
                const wantedData = _returnPost(response);

                const postEmbed = {
                    color: 0x00688B,
                    title: `${wantedData.title}. Posted on r/doge`,
                ...
                message.channel.send(`Something went wrong with r/doge`)
            }
        };
