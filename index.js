...
const inventoryCommandFiles = fs.readdirSync("./commands/inventory").filter(file => file.endsWith('.js'));
...
client.on('message', message => {
    ...
    con.query(`SELECT * FROM Noodle WHERE id = '${message.author.id}'`, (err, rows) => {
        ...
        if(rows.length < 1) { 
            sql = `INSERT INTO Noodle (id, xp, cur) VALUES ('${message.author.id}', ${generateXP()}, 0)`;
        } 
        else {
            let xp = rows[0].xp;
            sql = `UPDATE Noodle SET xp = ${xp + generateXP()} WHERE id = '${message.author.id}'`;
        }
...
//acm test server id, for adding a role after adding reaction
client.on("messageReactionAdd", async (reaction, user)=>{
    ...
    if(reaction.message.guild.id === '703342391909679157'){
        //put emoji on the reaction and the await is copy the id of the server roles in server settings
...
//acm test server id, for removing a role after removal of reaction
